...
export function useChainFilteredContracts(chainId: number) {
    ...
    const serviceManagers = useMemo(() => {
        return contracts.filter((c) => c.chainId === chainId && c.type === "CoverageProvider")
    }, [contracts, chainId])

    const coverageAgents = useMemo(() => {
        return contracts.filter((c) => c.chainId === chainId && c.type === "CoverageAgent")
    }, [contracts, chainId])

    const operatorProxies = useMemo(() => {
        return contracts.filter((c) => c.chainId === chainId && c.type === "EigenOperatorProxy")
    }, [contracts, chainId])
 ...
 */
export function useAvailableCoverageProviders(chainId: number, excludeIds: string[] = []) {
    const { contracts } = useContracts()

    const availableProviders = useMemo(() => {
        const excludeSet = new Set(excludeIds)
        return contracts.filter(
            (c) => c.type === "CoverageProvider" && c.chainId === chainId && !excludeSet.has(c.id)
        )
