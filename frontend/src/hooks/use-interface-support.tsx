...
const coverageProviderInterfaces: InterfaceName[] = Object.keys(coverageProviderMapping) as InterfaceName[]
    ...
    if (data) {
      for (let i = 0; i < coverageProviderInterfaces.length; i++) {
        ...
        if (queryResult?.status === "success" && Boolean(queryResult.result)) {
          return coverageProviderMapping[interfaceName as keyof typeof coverageProviderMapping]
        }
      }
    }
    return null
