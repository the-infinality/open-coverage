...
import { eigenOperatorProxyDeployAbi, eigenOperatorProxyBytecode } from "@/generated/eigen-operator-proxy-deployment"
 ...
 */
function OperatorProxyItem({
  operatorProxy,
}: {
  operatorProxy: CoverageContract
}) {
...
function DeployOperatorProxyForm({
  ...
  const [operatorName, setOperatorName] = useState(generateContractName("EigenOperatorProxy", operatorProxies))
  ...
  const deployedAddress = useMemo(() => {
    if (isSuccess && receipt) {
      ...
      if (contractAddress) {
        ...
        onSuccess(contractAddress, operatorName || `Operator Proxy ${contractAddress.slice(0, 8)}`)
    ...
    <div className="space-y-4">
      <div className="rounded-lg bg-muted/50 p-3">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          This will deploy a new EigenOperatorProxy contract. The handler will have administrative 
          control over the operator proxy.
        </p>
      </div>
      ...
      <div className="rounded-lg border bg-muted/30 p-3 space-y-2">
        <p className="text-xs font-medium text-muted-foreground">EigenLayer Addresses (from provider contract)</p>
        <div className="grid gap-1 text-xs">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-mono">{eigenAddresses.permissionController.slice(0, 10)}...</span>
          </div>
        </div>
      </div>
...
export function OperatorProxiesManagement({ contract }: OperatorProxiesManagementProps) {
  ...
  const chainId = isChainSupported
    ? (contract.chainId as SupportedChainId)
    : undefined
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Loading EigenLayer configuration...
          </CardDescription>
        </CardHeader>
    ...
    <Card>
          ...
          <Dialog open={isDeployDialogOpen} onOpenChange={setIsDeployDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[500px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  Deploy a new EigenOperatorProxy contract to manage operator registration and allocations.
                </DialogDescription>
              </DialogHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="rounded-lg bg-muted/30 p-3 space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Add an existing EigenOperatorProxy contract from the main Contracts page to manage it here.
          </p>
