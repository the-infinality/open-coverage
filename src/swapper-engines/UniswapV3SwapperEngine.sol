...
contract UniswapV3SwapperEngine is ISwapperEngine, UniswapV3SwapperEngineStorage {
    ...
    function swapForInput(
        ...
        // Execute the swap with explicit deadline
        ...
        returns (uint256 amountIn)
    {
        ...
        // For EXACT_OUT quote, path format is: output -> fee -> [intermediate -> fee ->]* input
        // So pathFirst should be the output token (base) and pathLast should be the input token (quote)
        bytes memory pathToUse;
        ...
        (bool success, bytes memory data) = base.staticcall(abi.encodeWithSelector(bytes4(0x313ce567)));
        if (success && data.length >= 32) {
            baseDecimals = abi.decode(data, (uint8));
        }

        uint256 unitAmount = 10 ** baseDecimals;
        uint256 unitAmountIn;
        (unitAmountIn,,,) = _getQuoter().quoteExactOutput(pathToUse, unitAmount);

        amountIn = unitAmountIn * amountOut / unitAmount;
    }
    // slither-disable-end uninitialized-local
